      ...
      $.ajax({
        ...
        dataType:'json',
          ...
          for (let i = 0; i < products.data.length; i++) {
            ...
            let img_url = 'http://localhost:8888/step8/public/storage/' + img_path;
            html = `
            <tr class="table-row" data-id="{{ $product->id }}">
              ...
              <td class="table-data"><a href="/step8/public/detail/${id}" class="btn btn-primary btn-sm">詳細</a></td>
              <td class="table-data">
                <form method="POST" action="/step8/public/delete/${id}" class="delete-form">
                    <button data-id="${id}" type="submit" data-url="/step8/public/delete/${id}" class="btn btn-danger btn-sm btn-delete">削除</button>
                </form>
              </td>
              <td class="table-data"><a href="/step8/public/cart/${id}" class="btn btn-warning btn-sm">購入</a></td>
            </tr>
          `;
            table.append(html);
            // テーブルソートを再適用
